	...
	devRemote.OnServerEvent:Connect(function(player, action, data)
		...
		if action == "SpawnKoala" then
			...
			local stageData = {
				...
				[2] = {age = 31, model = "Koala 1 year old", scale = 1.0},
				[3] = {age = 61, model = "Koala 2 year old", scale = 1.0},
				[4] = {age = 91, model = "Koala",            scale = 1.0},
			}
			...
			if template and player.Character then
				...
				local targetPos = player.Character:GetPivot() * CFrame.new(0, 5, -5)
				print("[DevService] SPAWN DEBUG:")
				print("  - Template WorldPivot Y: " .. template.WorldPivot.Position.Y)
				print("  - Target position Y: " .. targetPos.Position.Y)
				print("  - Player pivot Y: " .. player.Character:GetPivot().Position.Y)
				print("  - Offset: +5 studs above player")
				
				koala:PivotTo(targetPos)
				print("  - Koala HRP after PivotTo Y: " .. koala:FindFirstChild("HumanoidRootPart").Position.Y)
				...
				if (koala:GetPivot().Position - pos).Magnitude < 40 then
					...
					if stats and stats:FindFirstChild("Age") and stats:FindFirstChild("Stage") then
						if stats.Stage.Value < 4 then
							local thresholds = {31, 61, 91}
							stats.Age.Value = thresholds[stats.Stage.Value] or 91
